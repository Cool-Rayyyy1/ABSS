...
def FluxPipeline_call(
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            noise_pred_cpu = noise_pred.to("cpu")
...
#               • `"pil"` – returns a list of `PIL.Image.Image`
#               • `"np"`  – returns a NumPy array
#               • `"pt"`  – returns a torch tensor
#               • `"latent"` – returns latents
#         return_dict (`bool`, *optional*, defaults to `True`):
...
#     device = self._execution_device  # 一般是 cuda:0（即使启用 cpu_offload）
...
#     # 4. Prepare latent variables（在 execution_device 上，比如 cuda）
#     num_channels_latents = self.transformer.config.in_channels // 4
#     latents, latent_image_ids = self.prepare_latents(
#         batch_size * num_images_per_prompt,
#         num_channels_latents,
...
#     # 5. Prepare timesteps（在 execution_device 上，为 transformer 用）
#     sigmas = np.linspace(1.0, 1 / num_inference_steps, num_inference_steps)
#     image_seq_len = latents.shape[1]
#     mu = calculate_shift(
#         image_seq_len,
...
#     # **额外一步：把 scheduler 自己的 buffer 搬到 CPU，后面 step() 都在 CPU 上算**
#     if hasattr(self.scheduler, "sigmas") and isinstance(self.scheduler.sigmas, torch.Tensor):
#         self.scheduler.sigmas = self.scheduler.sigmas.to("cpu")
#     if hasattr(self.scheduler, "timesteps") and isinstance(self.scheduler.timesteps, torch.Tensor):
#         self.scheduler.timesteps = self.scheduler.timesteps.to("cpu")
...
#             # -------------------
#             # 6.1 给 transformer：用 CUDA 版 timestep
#             # -------------------
#             timestep_cuda = t.to(latents.device)
#             timestep_cuda = timestep_cuda.expand(latents.shape[0]).to(latents.dtype)
...
#             # -------------------
#             # 6.2 给 scheduler：全部转到 CPU 上 step 一步
#             # -------------------
#             latents_dtype = latents.dtype
#             latents_device = latents.device
...
def FluxTransformer2DModelForward(
    ...
    # ========= 关键补丁：对齐 device / dtype =========
    x_dev = self.x_embedder.weight.device
    x_dtype = self.x_embedder.weight.dtype

    hidden_states = hidden_states.to(device=x_dev, dtype=x_dtype)

    if encoder_hidden_states is not None:
        encoder_hidden_states = encoder_hidden_states.to(device=x_dev, dtype=x_dtype)

    if pooled_projections is not None:
        pooled_projections = pooled_projections.to(device=x_dev, dtype=x_dtype)

    if timestep is not None:
        timestep = timestep.to(device=x_dev)

    if guidance is not None:
        guidance = guidance.to(device=x_dev)

    if txt_ids is not None:
        txt_ids = txt_ids.to(device=x_dev)
    if img_ids is not None:
        img_ids = img_ids.to(device=x_dev)
    # ==============================================
    ...
    # 这里要注意 txt_ids / img_ids 可能是 None
    ...
    # FLUX 原本假设 txt_ids / img_ids 都存在
    # 如果你 pipeline 那边一直正常传，就不用额外判断
    ids = torch.cat((txt_ids, img_ids), dim=0)
    image_rotary_emb = self.pos_embed(ids)
